...
class OneAvisvalid extends Component {
    constructor(props) {
        super(props);
        this.state = { star:"" }
    }
    ...
    isholdavis=()=>
    {
      const {item}=this.props
       axios.put(`/enattenteavis/${item._id}/${item.enattente}`) 
      .then() 
      .catch()
    }
...
<MDBContainer>
  <MDBCard style={{ width: "22rem", marginTop: "1rem" }} id="cardavis">
    <MDBCardBody>
      ...
      <button type="button" className="btn btn-primary btn-lg " id="bntactiv"  onClick={this.isholdavis} >
            En attente
            </button>
